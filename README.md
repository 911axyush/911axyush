<h1 align="center">👋 Hey, I'm Aayush Jadhav</h1>
<h3 align="center">1st Year B.Tech Student • Game Developer • Roblox & Unity Creator</h3>

---
<img src="https://user-images.githubusercontent.com/74038190/225813708-98b745f2-7d22-48cf-9150-083f1b00d6c9.gif" width="500">
---
### 🚀 About Me
- 🎮 Game Developer (Roblox + Unity + JS)
- 🧠 Learning: LUA, C#, Python & Game Design  
- 💡 Building mini-games, prototypes & fun experiments  
- 🌱 Currently improving: Multiplayer systems, UI/UX, and AI NPC logic  
- 🎯 Goal 2025: Publish my first full indie game  

---

### 🔥 Tech Stack
**Languages:**  
`JavaScript` `Lua` `C#` `Python`  

**Tools:**  
`Unity` `Roblox Studio` `Blender` `Figma` `Git`

**Game Dev:**  
`Level Design` `AI Behaviour` `Multiplayer` `Animations`

---


### 🔗 Connect with Me
<p align="left">
  <a href="https://instagram.com/911axyush" target="_blank"><img src="https://skillicons.dev/icons?i=instagram" height="40"/></a>
  <a href="www.linkedin.com/in/aayush-jadhav-877649349" target="_blank"><img src="https://skillicons.dev/icons?i=linkedin" height="40"/></a>
</p>


